        ...
        if num_agents == 16:
            for i, agent in enumerate(world.agents):
                if i < len(world.agents)/2:
                    ...
                    agent.x_pos = uniformpoint.evenly_distributed_points_16()[i][0]
                    agent.y_pos = uniformpoint.evenly_distributed_points_16()[i][1]
                    # agent.x_pos = uniformpoint.generate_points(num_agents, world.radius, min_dist)[i][0]
                    # agent.y_pos = uniformpoint.generate_points(num_agents, world.radius, min_dist)[i][1]
                    points_urllc.append((agent.x_pos, agent.y_pos))
                else:
                    agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_16()[int(i-len(world.agents)/2)][0] + 10
                    agent.y_pos = uniformpoint.evenly_distributed_points_16()[int(i-len(world.agents)/2)][1] + 10
                    # agent.x_pos = uniformpoint.generate_points(num_agents, world.radius, min_dist)[int(i-len(world.agents)/2)][0] + 10
                    # agent.y_pos = uniformpoint.generate_points(num_agents, world.radius, min_dist)[int(i-len(world.agents)/2)][1] + 10
                    points_embb.append((agent.x_pos, agent.y_pos))
        if num_agents == 20:
            for i, agent in enumerate(world.agents):
                if i < len(world.agents)/2:
                    agent.name = 'URLLC UE %d' % (i+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_20()[i][0]
                    agent.y_pos = uniformpoint.evenly_distributed_points_20()[i][1]
                    points_urllc.append((agent.x_pos, agent.y_pos))
                else:
                    agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_20()[int(i-len(world.agents)/2)][0] + 10
                    agent.y_pos = uniformpoint.evenly_distributed_points_20()[int(i-len(world.agents)/2)][1] + 10
                    points_embb.append((agent.x_pos, agent.y_pos))
        if num_agents == 24:
            for i, agent in enumerate(world.agents):
                if i < len(world.agents)/2:
                    agent.name = 'URLLC UE %d' % (i+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_24()[i][0]
                    agent.y_pos = uniformpoint.evenly_distributed_points_24()[i][1]
                    points_urllc.append((agent.x_pos, agent.y_pos))
                else:
                    agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_24()[int(i-len(world.agents)/2)][0] + 10
                    agent.y_pos = uniformpoint.evenly_distributed_points_24()[int(i-len(world.agents)/2)][1] + 10
                    points_embb.append((agent.x_pos, agent.y_pos))
        if num_agents == 28:
            for i, agent in enumerate(world.agents):
                if i < len(world.agents)/2:
                    agent.name = 'URLLC UE %d' % (i+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_28()[i][0]
                    agent.y_pos = uniformpoint.evenly_distributed_points_28()[i][1]
                    points_urllc.append((agent.x_pos, agent.y_pos))
                else:
                    agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_28()[int(i-len(world.agents)/2)][0] + 10
                    agent.y_pos = uniformpoint.evenly_distributed_points_28()[int(i-len(world.agents)/2)][1] + 10
                    points_embb.append((agent.x_pos, agent.y_pos))
        if num_agents == 32:
            for i, agent in enumerate(world.agents):
                if i < len(world.agents)/2:
                    agent.name = 'URLLC UE %d' % (i+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_32()[i][0]
                    agent.y_pos = uniformpoint.evenly_distributed_points_32()[i][1]
                    points_urllc.append((agent.x_pos, agent.y_pos))
                else:
                    agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                    agent.x_pos = uniformpoint.evenly_distributed_points_32()[int(i-len(world.agents)/2)][0] + 10
                    agent.y_pos = uniformpoint.evenly_distributed_points_32()[int(i-len(world.agents)/2)][1] + 10
                    points_embb.append((agent.x_pos, agent.y_pos))
        # add landmarks
