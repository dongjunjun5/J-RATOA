    ...
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            if i < len(world.agents)/2:
                ...
                agent.x_pos = uniformpoint.evenly_distributed_points_1vs9()[i][0]
                agent.y_pos = uniformpoint.evenly_distributed_points_1vs9()[i][1]
                points_urllc.append((agent.x_pos, agent.y_pos))
            else:
                agent.name = 'eMBB UE %d' % (i-len(world.agents)/2+1)
                agent.x_pos = uniformpoint.evenly_distributed_points_1vs9()[int(i - len(world.agents) / 2)][0] + 10
                agent.y_pos = uniformpoint.evenly_distributed_points_1vs9()[int(i - len(world.agents) / 2)][1] + 10
